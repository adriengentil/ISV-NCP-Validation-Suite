...
"""Stop an AWS bare-metal EC2 instance and verify it reaches the stopped state.

Stops the instance using the EC2 API and waits for it to reach the "stopped"
state. Bare-metal instances require longer waiter timeouts than VMs due to
hardware power-down (IPMI, BMC, full POST cycle).
...
def main() -> int:
    """Stop a bare-metal EC2 instance and wait for it to reach stopped state.

    Returns:
        0 on success, 1 on failure
    """
    parser = argparse.ArgumentParser(description="Stop bare-metal EC2 instance")
    ...
    try:
        ...
        # ============================================================
        print(f"Stopping bare-metal instance {args.instance_id}...", file=sys.stderr)
        ...
        # Bare metal needs longer waiter: full hardware power-down cycle
        # ============================================================
